...
namespace HeThongQuanLyKhachSan
{
    internal class QuanLy
    {
        ...
        {
            long tong_tien = 0;
            string truy_van = "select hoa_don.ID_don_dat_phong from don_dat_phong join hoa_don on don_dat_phong.ID_don_dat_phong = hoa_don.ID_don_dat_phong where ngay_thanh_toan >= '" + ngayBatDau + "' and ngay_thanh_toan <= '" + ngayKetThuc + "'";
            ThaoTacVoiSQL thaoTacVoiSQL = new ThaoTacVoiSQL();
            thaoTacVoiSQL.Truy_van = truy_van;
            MySqlDataReader reader = thaoTacVoiSQL.layDuLieuChoClass();
            if (reader.Read())
        ...
        {
            string truy_van = "select phong.ID_phong, so_phong, count(chi_tiet_don_dat_phong.ID_don_dat_phong) as so_don_dat_phong from phong join chi_tiet_don_dat_phong on phong.ID_phong = chi_tiet_don_dat_phong.ID_phong join don_dat_phong on chi_tiet_don_dat_phong.ID_don_dat_phong = don_dat_phong.ID_don_dat_phong where ngay_nhan_phong >= '" + ngayBatDau + "' and ngay_nhan_phong <= '" + ngayKetThuc + "' and trang_thai_don = 'Đã trả' group by chi_tiet_don_dat_phong.ID_phong order by so_don_dat_phong desc";
            ThaoTacVoiSQL thaoTacVoiSQL = new ThaoTacVoiSQL();
            thaoTacVoiSQL.Truy_van = truy_van;
            return thaoTacVoiSQL.layDuLieuChoGridView();
        }
        ...
        {
            string truy_van = "select ho_ten, so_dien_thoai, count(don_dat_phong.ID_don_dat_phong) as so_lan_dat_phong from khach_hang join don_dat_phong on khach_hang.ID_khach_hang = don_dat_phong.ID_khach_hang where ngay_nhan_phong >= '" + ngayBatDau + "' and ngay_nhan_phong <= '" + ngayKetThuc + "' and trang_thai_don = 'Đã trả' group by don_dat_phong.ID_khach_hang order by so_lan_dat_phong desc";
            ThaoTacVoiSQL thaoTacVoiSQL = new ThaoTacVoiSQL();
            thaoTacVoiSQL.Truy_van = truy_van;
            return thaoTacVoiSQL.layDuLieuChoGridView();
        }
